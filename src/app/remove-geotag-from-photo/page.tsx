...
const config = toolLandings.find((t) => t.slug === "remove-geotag-from-photo")!;
...
const faqs = [
      {
        ...
        answer: "A geotag is GPS metadata embedded in a photo that records the exact latitude, longitude, and sometimes altitude where the image was captured, along with a timestamp.",
      },
      {
        question: "Do social media platforms strip geotags?",
        answer: "Some platforms compress or remove certain metadata, but you should not rely on them for privacy. Always remove geotags yourself before uploading to guarantee your location is protected.",
      },
      {
        question: "Can geotags be recovered after removal?",
        answer: "No, once geotags are removed from a photo, they cannot be recovered from the cleaned file. Always keep an original copy if you need the location data for personal records.",
      },
      {
        question: "Are geotags the same as location data?",
        answer: "Yes, geotags and location data refer to the same thing — GPS coordinates embedded in image metadata that reveal where a photo was taken.",
      },
      {
        question: "Should I remove geotags from all my photos?",
        answer: "It is best practice to remove geotags from photos before sharing them publicly, especially on social media, forums, or online marketplaces where strangers can access them.",
      }
];


export default function RemoveGeotagFromPhotoPage() {
