...
const config = toolLandings.find((t) => t.slug === "remove-metadata-from-jpeg")!;
...
const faqs = [
      {
        question: "What is the difference between JPEG and JPG metadata?",
        answer: "JPEG and JPG use the same EXIF metadata format. The only difference is the file extension — JPEG metadata contains the same camera, GPS, and date information as JPG files.",
      },
      {
        question: "Will my JPEG file size change after cleaning?",
        answer: "Removing metadata typically reduces the JPEG file size slightly since the embedded data blocks are stripped, though the change is usually negligible for most images.",
      },
      {
        question: "Can I preview metadata before removing it?",
        answer: "Yes, MetaClean lets you view all embedded metadata in your JPEG files before you decide what to remove, giving you full control over your privacy.",
      },
      {
        question: "Does this work on JPEG files from any camera?",
        answer: "Yes, the tool works with JPEG files from any camera, smartphone, or device. It reads and removes standard EXIF and IPTC metadata regardless of the source.",
      },
      {
        question: "Is the JPEG metadata removal process reversible?",
        answer: "No, metadata removal is permanent. Once stripped, the original metadata cannot be recovered, so keep an unmodified copy if you need the original EXIF data.",
      }
];


export default function RemoveMetadataFromJpegPage() {
